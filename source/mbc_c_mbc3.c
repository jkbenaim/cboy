...
void mbc_c_mbc3_read_bank_n() {
  if( !cart.cartromValid_bank_n[address-0x4000] )
  {
    ...
    for( readAddress = startAddress; readAddress <= endAddress; readAddress++ )
    {
      ...
      data = (u8)temp;
//       printf("a%02x\n", temp);
      cart.cartrom_bank_n[readAddress-0x4000] = data;
      cart.cartromValid_bank_n[readAddress-0x4000] = 1;
    }
    fgetc(cart.fd);
    fgetc(cart.fd);
  }
...
void mbc_c_mbc3_read_extram() {
  ...
  if( !cart.extram_bank_validRead[address-0xA000] )
  {
    ...
    for( readAddress = startAddress; readAddress <= endAddress; readAddress++ )
    {
      ...
      data = (u8)temp;
//       printf("a%02x\n", temp);
      if( cart.extram_bank_validRead[readAddress-0xA000] == 0 )
      {
        cart.extram_bank[readAddress-0xA000] = data;
        cart.extram_bank_validRead[readAddress-0xA000] = 1;
      }
    }
    fgetc(cart.fd);
    fgetc(cart.fd);
...
void mbc_c_mbc3_write_extram() {
  ...
  if( (cart.extram_bank_validWrite[address&0x1fff] == 0)
      && (oldData != newData) )
